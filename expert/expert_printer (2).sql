...
--
-- Host: localhost
-- Generation Time: Nov 13, 2018 at 10:58 AM
...
(10, '10', 'Is the printer producing active light when put on?', 'Was the printer output clear enough before now?', 'Where you having power problems with your printer?', 'Have you checked if the printer is accepting other paper  than the one you have?', 'Check If the printer is well configured, also check the power if the connection is proper!', 'Your printer needs maintenance: Simply engage the printer in a thorough maintenance by opening the cartridge, clean it. check the power cables and ensure it is fine then clean the parts. Also, refill the ink if it is getting less. '),
...
(17, '17', 'Is printer having low power supply?', 'Does this issue come up often?', 'Have you checked if the printer settings is resulting the delay?', 'Most times, when printer becomes very dirty inside this issue comes up, have you checked inside to confirm if dirt is causing it?', 'Clean the printer thoroughly....If the problem persists then take it to a printer engineer!', 'Clean printer, check the printer configuration and also ensure the printer power is sufficient!');
